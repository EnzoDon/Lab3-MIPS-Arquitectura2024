...
def connect_auto():
    ...
    # Si hubiera más de uno, elegimos el que más pinta de USB/UART tenga; si no, el primero.
    if len(ports) > 1:
        candidates = [p.device for p in ports
                      if any(tag in (p.description or '').lower()
                             for tag in ['usb', 'uart', 'basys', 'ftdi', 'cp210', 'ttyusb', 'com'])]
        selected_port = candidates[0] if candidates else ports[0].device
        log_message(f"Multiple ports found. Using: {selected_port}")
    else:
...
# Cambios en los colores principales
BG = "#f5f5dc"          # Hueso
PANEL_BG = "#f5f5dc"    # Hueso para paneles también
TEXT_COLOR = "#000000"  # Texto en negro para mejor contraste
ACCENT = "#ff6961"      # Rojo pastel para headers o detalles

style = ttk.Style()
style.theme_use("clam")
style.configure("TFrame", background=BG)
style.configure("TLabel", background=BG, foreground=TEXT_COLOR)
style.configure("TButton", background=ACCENT, foreground="#ffffff", padding=6, relief="flat")
style.map("TButton", background=[('active', '#ff8a80')], foreground=[('pressed', '#ffffff'), ('active', '#ffffff')])
...
ttk.Button(group1, text="CONNECT", command=connect_auto).pack(pady=2, fill='x')
